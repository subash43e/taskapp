...
export async function getTodayTasks(userId: string): Promise<Task[]> {
  ...
  return tasks.sort((a, b) => (a.dueTime || '23:59').localeCompare(b.dueTime || '23:59'));
}
...
export async function toggleTaskCompletion(userId: string, taskId: string): Promise<void> {
  if (!userId) throw new Error('User must be authenticated to update tasks');
...
export async function searchUserTasks(userId: string, searchQuery: string): Promise<Task[]> {
